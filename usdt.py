...
UPI_ID = os.getenv("UPI_ID", "9548511172@finobank").strip()
...
try:
    ADMINS = [int(x.strip()) for x in os.getenv("ADMINS", "1888507338, 8564072723").split(",") if x.strip()]
    if not ADMINS: raise ValueError
except Exception:
    raise ValueError("CRITICAL ERROR: 'ADMINS' missing or invalid format in .env")
try:
    ADMIN_GROUP_ID = int(os.getenv("ADMIN_GROUP_ID", "-1004426384667"))
    if ADMIN_GROUP_ID == 0: raise ValueError
except Exception:
    raise ValueError("CRITICAL ERROR: 'ADMIN_GROUP_ID' missing or invalid
in .env")
...
FOOTER_TAG = "⚡  <i>Powered by USDT Ecosystem • Secured Node</i>"
# Validations Regex
...
def verify_trc20_address(address: str) -> bool:
    """Performs Base58Check decode structural validation for TRON address
geometry."""
...
async def async_expire_worker(order_id: int, application, delay: float = 1800.0):
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT user_id FROM orders WHERE order_id=?",
(order_id,))
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == "await_amount":
        ...
        if usdt < LIMITS['MIN'] or usdt > LIMITS['MAX']:
            return await update.message.reply_text(f"🚨 <b>Boundary Breach!</b> Range limit is <code>{LIMITS['MIN']}-{LIMITS['MAX']}</code> USDT.",
parse_mode="HTML")
...
"""
    ...
    if stage == "await_utr_explicit":
        ...
        if not re.match(RE_UTR, utr):
            return await update.message.reply_text("❌  <b>Verification Discrepancy:</b> UPI Network rules mandate a strict 12 to 22-digit numerical
sequence. Re-enter correctly.", parse_mode="HTML")
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data.startswith("USER:SUBMIT:"):
        ...
        await q.message.reply_text(f"📝 <b>ORDER TRACKING INITIALIZED FOR
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage != "await_screenshot_explicit" or not target_order_id:
        return await update.message.reply_text("❌  <b>Flow Violation:</b>
Please tap the 'Link UTR & Upload Proof' button on your order invoice before uploading your snapshot receipt.", parse_mode="HTML")
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT usdt, network, wallet, amount, escrow, utr
FROM orders WHERE order_id=?", (target_order_id,))
        row = cursor.fetchone()
    if not row:
...
🛡️ <b>Structural OTC Core Protocol:</b> {'Secure Escrow Node Balance' if
escrow else 'Direct Balance Settlement'}
━━━━━━━━━━━━━━━━━━━━━━━━━━
